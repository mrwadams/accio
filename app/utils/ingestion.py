    ...
    def process_file(
        ...
        try:
            ...
            if splitter_type == "semantic":
                # For semantic chunking, embeddings are already generated by LangChain
                embedded_chunks = []
                for chunk, doc_chunk in zip(chunks, doc_chunks):
                    embedding = chunk["metadata"].get("embedding")
                    if embedding is None:
                        raise ValueError("Semantic chunking did not return embeddings in chunk metadata.")
                    from app.utils.embeddings import EmbeddedChunk
